...
#%%            
    def redimensionarSenal(self,senal):                                                     # Genera senalContinua en funcion de la dimension
        if senal.ndim == 1:                
            self.x_max = len(senal);                                                        # ndim = 1 : Solo genera su rango de tiempo
        ...
        elif senal.ndim == 2:                                                               # ndim = 2: Solo genera su rango de tiempo, la
        ...
        elif senal.ndim == 3:                                                               # ndim = 3: Genera un rango de tiempo y                                                                                   
...
#%%
    def nextPow2(self,numero):
        potencia=1;
        argumento = True;
        while argumento:
            if 2**potencia >= numero:
                argumento = False;
            else:
                potencia = potencia + 1;
    ...
    def calcularPSD(self,metodo,fs):
        if self.senalFiltrada.shape[0] == 0:                                                # Sí ya hay una señal filtrada
            ...
            senal = self.senalFiltrada;
                                                                                            # MECANISMO DE ACCIÓN
        senal = senal - np.mean(senal);                                                     # Se elimina el nivel DC de la señal.
        N = senal.shape[0];                                                                 # Determina la cantidad de muestras para
        resolucion = fs/N;                                                                  # establecer la resolucion de frecuencias,
        
        frecuencias = np.arange(0.0,fs,resolucion);                                         # Se genera el respectivo vector de frecuencias,
        armonicos = np.abs(fft(senal));                                                     # luego se calculan los armónicos de la señal,
        PSD_crudo = np.power(armonicos,2)/N;                                                # y finalmente se calcula el periodograma de FORMA DIRECTA
        
        if metodo == '|Armónicos|':
            self.__controlador.graficarPSD(frecuencias,armonicos);
        elif metodo == 'PSD crudo':
            self.__controlador.graficarPSD(frecuencias,PSD_crudo);
...
#%%                                
    def calcularPSD_Welch(self,fs,tipoVentana,tamanoVentana,solapamiento,escala):
        if self.senalFiltrada.shape[0] == 0:                                                # Sí ya hay una señal filtrada
            ...
            senal = self.senalFiltrada;                                                     # MECANISMO
                                                                                            # Se leen los parámetros capturados por la vista
        senal = senal - np.mean(senal);                                                     # se quita el nivel DC de la señal y se calcula
        nextPow2 = self.nextPow2(tamanoVentana);                                            # la siguiente potencia de 2 del tamaño de la
                                                                                            # ventana para ingresar todos los parámetros
        if (solapamiento <= tamanoVentana) and (tamanoVentana < senal.shape[0]):            # al algoritmo de Welch
            frecuenciasWelch, PSD_Welch = signal.welch(senal,fs,tipoVentana,tamanoVentana,solapamiento,nextPow2,return_onesided=False,scaling=escala);
            self.__controlador.graficarPSD(frecuenciasWelch,PSD_Welch);
